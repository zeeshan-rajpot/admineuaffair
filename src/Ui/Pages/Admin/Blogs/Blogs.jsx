...
const Blogs = () => {
  ...
  const handleImageChange = async (e) => {
    ...
    if (file) {
      if (file.size > maxSizeBytes) {
        const errorMessage = `File size exceeds the limit of ${maxSizeMB} MB.`;
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const upload = await userApi.uploadBlogs(uploadData);
      console.log(upload);
      fetchArticles();
...
{isModalOpen && (
      <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center mx-auto z-50 overflow-y-auto max-h-full">
        <div ref={modalRef} className="bg-white p-8 shadow-lg relative w-full md:w-1/2 rounded-3xl">
          <button
