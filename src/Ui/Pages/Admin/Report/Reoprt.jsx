...
const Report = () => {
  ...
  const fetchData = async () => {
    try {
      const data = await userApi.getusersDetail(); // Fetch data
      setEmailList(data);
      paginateEmails(data, 1); // Initially paginate the first page
    } catch (error) {
  ...
  const paginateEmails = (emails, page) => {
    ...
    setDisplayedEmails(paginatedEmails);
    setCurrentPageemail(page);
  };

  const handleSearch = (query) => {
    setSearchQuery(query);
    if (query) {
      const filteredEmails = emailList.filter(emailItem =>
        emailItem.email.toLowerCase().includes(query.toLowerCase())
      );
      setDisplayedEmails(filteredEmails);
    } else {
      paginateEmails(emailList, 1); // Show the first page of emails when the query is cleared
    }
  };
            ...
            <div className='w-full mb-4'>
              <input
                ...
                onChange={(e) => {
                  ...
                  handleSearch(e.target.value); // Trigger the search as the user types
                }}
                onClick={() => handleSearch(searchQuery)} // Trigger search when the input is clicked
              />

            </div>
            {displayedEmails.length > 0 && (
  <ul className="border border-gray-400 p-2 rounded-md max-h-40 overflow-y-auto">
    {displayedEmails.map((emailItem, index) => (
      <li
        key={index}
